...
namespace JHchoi.Contents
{
	public class ContentsLoader : MonoSingleton<ContentsLoader>
	{
		...
		public IEnumerator Load(string gameName, string contentName, OnComplete loadComplete)
		{
			...
			else
			{
                ...
                var fullpath = string.Format("Contents/" + gameName + "Contents/" + "{0}", contentName);
				yield return StartCoroutine(ResourceLoader.Instance.Load<GameObject>(fullpath, o => SetupContent(o, gameName, contentName, loadComplete)));
		...
		void SetupContent(Object o, string gameName, string contentName, OnComplete loadComplete)
		{
			if (!_contentsMap.ContainsKey(contentName))
			{
				...
				contentScript.Load(gameName, OnLoadComplete);
				AddEvent(contentName, loadComplete);
		...
		public void Unload(string gameName, string contentName)
		{
			IContent contents;
		...
		public void ForceUnload(string gameName, string contentName)
		{
			IContent contents;
			_contentsMap.TryGetValue(contentName, out contents);
			if (contents != null)
