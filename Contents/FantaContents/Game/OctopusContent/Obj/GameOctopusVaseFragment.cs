...
public class GameOctopusVaseFragment : MonoBehaviour
{
    List<Rigidbody> m_pList = null;
    List<Vector3> m_pList_Trans = null;

    private void Awake()
    {
        m_pList = new List<Rigidbody>();
        m_pList_Trans = new List<Vector3>();
        Transform frags = transform.Find("fragments");
        foreach (Transform obj in frags)
        {
            m_pList_Trans.Add(obj.localPosition);
    ...
    public void VasePosInit()
    {
        for (int i = 0; i < m_pList.Count; i++)
        {
            m_pList[i].transform.localPosition = m_pList_Trans[i];
        }
    }
